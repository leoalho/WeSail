...
const addCrew = async (
  ...
  if (
    boat &&
    user &&
    !boat.crew.includes(userObjectId) &&
    (await checkOWner(boatId, ownerId))
  ) {
    await boat.updateOne({
      $addToSet: { crew: userObjectId },
      $pull: { crewRequests: userObjectId },
    });
    await user.updateOne({
      $addToSet: { crewMember: boatObjectId },
      $pull: { crewRequestsPending: boatObjectId },
    });
    await boat.save();
    await user.save();
  }
...
export const removeFromBoatArray = async (
  id: string,
  field: BoatArray,
  value: string
) => {
...
export const addToBoatArray = async (
  id: string,
  field: BoatArray,
  value: string
) => {
...
export const boatJsonPatch = async (
  adderId: string | undefined,
  boatId: string,
  patches: Patch[]
) => {
