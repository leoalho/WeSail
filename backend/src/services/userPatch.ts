...
export interface BoatSideEffect {
  field: UserArray;
  field2: BoatArray;
}
...
const addFriend = async (userId: string, friendId: string) => {
  ...
  if (user && friend && user.friendRequests.includes(friendObjectId)) {
    ...
    await friend.updateOne({
      ...
      $pull: { friendRequestsPending: UserObjectId },
    });
    await user.save();
    await friend.save();
  }
...
const addFriendRequest = async (userId: string, friendId: string) => {
  ...
  if (user && friend && !user.friends.includes(friendObjectId)) {
    await user.updateOne({
      $addToSet: { friendRequestsPending: friendObjectId },
    });
...
const addCrewRequest = async (userId: string, boatId: string) => {
  ...
  if (
    boat &&
    user &&
    !user.crewMember.includes(boatObjectId) &&
    !user.boats.includes(boatObjectId)
  ) {
